    ...
    private void start(String[] args) throws Exception {
        ...
        if (args.length > 0) {
            ...
            if (params.get("mongo") != null) {
                MongoDBCXN = params.get("mongo").get(0);
            }
            if (params.get("service") != null) {
                ServiceURL = params.get("service").get(0);
            }
            if (params.get("console") != null) {
                bWriteToConsole = true;
            }
            if (params.get("database") != null) {
                sDatabaseName = params.get("database").get(0);
            }
            if (params.get("collection") != null) {
                sCollectionName = params.get("collection").get(0);
            }
            if (params.get("objectname") != null) {
                sObjectName = params.get("objectname").get(0);
            }
        ...
        try (MongoClient mongoClient = MongoClients.create(MongoDBCXN)) {
            ...
            boolean collectionExists = database.listCollectionNames().into(new ArrayList<String>())
                    .contains(sCollectionName);

            // If collection doesn't exist, create it as a time-series collection
            if (!collectionExists) {
                System.out.println(
                        "\"" + sCollectionName + "\" collection not found, creating new time series collection");
                ...
                System.out.println("\n\"" + sCollectionName
                        + "\" collection exists!  For best perfomanceuse a time series collection.");
            }
            ...
            while (true) {
                for (ObjectInstance c : objectInstanceNames) {
                    ...
                    // Future note: May need to tweak this getKeyProperty as the Kafka Connector
                    // exposes "type" but not all MBeans use that some use "name"
                    String mbean_name = x.getKeyProperty("type");
                    MBeanInfo info = mbsc.getMBeanInfo(x);
                    MBeanAttributeInfo[] attribute = info.getAttributes();
                    Document new_doc = new Document().append("_id", new ObjectId())
                            .append("sample_time", new Date()).append("mbean", mbean_name);
                    ...
                    for (MBeanAttributeInfo attr : attribute) {
                        ...
                        if (attr.isReadable()) {
                            if (bWriteToConsole)
                                System.out.println(attr.getName() + " = " + attributeValue.toString()); // mbsc.getAttributes(x,
                                                                                                        // new
                                                                                                        // String[]{attr.getName()}));
                    ...
                    System.out.print("\rTotal data points written " + i + " (last written: "
                            + sdfLastWritten.format(dLastWritten.getTime()) + ")");
